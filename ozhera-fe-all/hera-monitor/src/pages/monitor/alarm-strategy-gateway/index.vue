<template>
  <div class="unusual-box">
    <lone-module class="title">
      <span class="title-text">网关报警策略</span>
    </lone-module>
    <div class="inner">
        ...
        <div class="detail-box">
          <template>
            <el-table :data="tableData" v-loading='loading'>
              <el-table-column prop="strategyName" :label="$t('policyPage.policyName')" width="250px"></el-table-column>
              <el-table-column prop="appName" :label="$t('appName')" width="130px">
                <template slot-scope="scope" >
                  <AppName :appName="scope.row.appName" :teslaOptions="teslaOptions"/>
                </template>
              </el-table-column>
              <el-table-column prop="alarmRulesArr" :label="$t('policyPage.alarmRule')"  width="400px">
                <template slot-scope="scope">
                  <el-popover
                    placement="top-start"
                    trigger="hover"
              ...
              <el-table-column prop="alertTeam" label="通知组" width="200px">
                <template slot-scope="scope">
                  <p v-for="team in JSON.parse(scope.row.alertTeam)" :key="team.id">{{team.name}}</p>
                </template>
              </el-table-column>
              <el-table-column prop="creater" :label="$t('updater')" width="130px"> </el-table-column>
              <el-table-column prop="createTime" :label="$t('updateTime')" sortable width="180px">
                <template slot-scope="scope">
...
import AppName from '@/pages/monitor/alarm-strategy-gateway/components/AppName.vue';
import {
  transformRes,
  retrunDetail,
  transformStr,
  returnStr
} from '@/pages/monitor/alarm-strategy-gateway/transform.js'
...
export default {
  ...
  computed:{
    ...mapState({
      ...
      teslaOptions: state => state.moduleStrategyGateway.teslaOptions, // 报警频率
    }),
  },
  data() {
    return {
  ...
  async mounted() {
    await this.GETTESLAOPTIONS(); // 获取tesla指标
    await this.GETRUlESOPTION(); //获取所有规则的选项
    await this.searchList();
  },
  methods: {
    ...mapActions('moduleStrategyGateway', ['GETTESLAOPTIONS']), // 获取基础指标
    ...mapActions('moduleStrategyList', ['GETRUlESOPTION']),
    searchList(){
      this.loading = true;
      return serverStrategy.strategyListApi(this.searchParams).then(res=>{
    ...
    deleteItem(row){
      let appName = this.teslaOptions.filter(item=>item.projectKey ===row.appName )[0]?.projectCName || row.appName;
      this.$confirm(`确定删除 ${appName} 的报警规则吗?`, this.$t('prompt'), {
