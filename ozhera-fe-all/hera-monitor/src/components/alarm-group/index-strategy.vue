...
<template>
  <div class="alarm-group-box">
    ...
    <div class="table-box">
      ...
      <el-table-column
        prop="memeberIds"
        :label="$t('policyPage.NotifiRecipient')">
      ...
      <template slot-scope="scope">
        ...
        <AddAtMmbers @click="copyMembers(scope.row.members)" v-if="showCopyM" type="text"/>
      </template>
    </el-table-column>
    </el-table>
    </div>
...
export default {
  ...
  methods:{
    ...
    copyMembers(members){
      let names = members.map(item=>item.name)
      this.$emit('AddAtMembers',names);
    }
  }
}
</script>
