<template>
  <el-select v-model="value" multiple :placeholder="placeholder || $t('pleaseSel')" :size="size"
             ref="select"
             popper-class="dash-board-select"
             @change="changeHandle"
             @visible-change="visibleChange">
    <el-option :value="checkAll"
               :class="{'selected': checkAll}"
               v-if="isShowCheckAll && options && (options.length != 1 ||
               (options.length == 1 && valueKey ? options[0][valueKey] != this.mutexOption : options[0] != this.mutexOption))">
      <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll"
                   style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; padding-left: 10px"
                   @change="checkAllChange">{{ $t('selectAll') }}</el-checkbox>
    </el-option>
    <el-option
      ...
      <el-checkbox :value="value.includes(item)"
                   v-if="multiple && isShowCheckAll"
                   style="width: 100%"
                   @change="handleCheckedChange(item)">{{item}}</el-checkbox>
    </el-option>
  </el-select>
</template>
<script>
export default {
  props: {
    ...
    labelKey: { // 下拉选项label 标识的键名
      type: String,
      default: 'label'
    },
    valueKey: { // 下拉选项value 标识的键名
      type: String,
      default: 'value'
    },
    mutexOption: { // 互斥选项
